...
Navigate to `crates/wpt-runner` and run `cargo run` to start the WPT Runner. You can optionally pass a path to a specific file or directory to run the tests on. For example, `cargo run -- ../../tools/wpt/fetch/api/headers/header-setcookie.any.js`
